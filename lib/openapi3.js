        ...
        if (!p.startsWith('x-')) {
            for (var m in source.paths[p]) {
                if ((m !== 'parameters') && (m !== 'summary') && (m !== 'description') && (!m.startsWith('x-'))) {
                    ...
                    var sMethodUniqueName = (method.operation.operationId ? method.operation.operationId : m.toUpperCase() + ' ' + p).split('?')[0].split('/').slice(3).join('/');
...
function getParameters(data) {
    ...
    for (let param of data.parameters) {
        if (typeof param.schema.default === "string") {
            param.schema.default = param.schema.default.split('|').join('&#124;')
        }
        if (Array.isArray(param.schema.default)) {
            param.schema.default = param.schema.default.map(item => {return item.split('|').join('&#124;')})
        }
        param.exampleValues = {};
        ...
        if ((typeof param.description !== 'undefined') && (typeof param.description === 'string')) {
            ...
            param.shortDesc = param.shortDesc.split('|').join('&#124;')
        }
        if (param.in === 'cookie') {
        ...
        if (param.in === 'path') {
            ...
            exampleUriTemplateStr = exampleUriTemplateStr.split('{' + param.name + '}').join(template);
        }
        if ((param.in === 'query') && (param.name)) {
            ...
            if (param.example) {
                let isFirstExample = exampleUriTemplateStr.indexOf('{?') < 0;
                let exPrefix = isFirstExample ? '{?' : '{&';
                exampleUriTemplateStr += (exPrefix + template);
                data.exampleParameters.push(param);
            }
    ...
    data.exampleQueryString = data.exampleUriExample.substr(data.exampleUriExample.indexOf('?'));
    if (!data.exampleQueryString.startsWith('?')) data.exampleQueryString = '';
    data.exampleQueryString = data.exampleQueryString.split('%25').join('%');

    if (typeof data.operation.description === "string") {
        data.operation.description = data.operation.description.split('\n').join('<br/>')
    }
}
